...
static bool is_privileged(const struct wl_global *global) {
	...
	return
		...
		global == server.image_copy_capture_manager_v1->global ||
...
bool server_init(struct sway_server *server) {
	...
	server->image_copy_capture_manager_v1 = wlr_ext_image_copy_capture_manager_v1_create(server->wl_display, 1);
